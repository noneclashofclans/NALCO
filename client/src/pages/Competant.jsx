...
const AUTHORITY_HISTORY_KEY = "authority_action_history";
...
const Competant = () => {
  ...
  const [actionHistory, setActionHistory] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(AUTHORITY_HISTORY_KEY) || "[]");
    } catch {
      return [];
    }
  });

  const saveHistoryEntry = (req, actionType, authorityRemarks) => {
    const entry = {
      id: req._id,
    ...
    setActionHistory(prev => {
      ...
      localStorage.setItem(AUTHORITY_HISTORY_KEY, JSON.stringify(updated));
      return updated;
    });
  };
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (window.confirm("Clear all action history?")) {
                          ...
                          localStorage.removeItem(AUTHORITY_HISTORY_KEY);
                        }
                      }}
                    >
                      Clear history
